...
function validateAmount(amount, paramName = 'amount', options = {}) {
  const { allowZero = false, minValue = 0, maxValue = Number.MAX_VALUE } = options;
...
function validateTransactionHash(txHash, paramName = 'txHash') {
  ...
  if (!/^0x([A-Fa-f0-9]{64})$/.test(txHashStr)) {
    throw new Error(
      `Invalid ${paramName}: ${txHash}. Must be a valid Ethereum transaction hash.`
    );
  }
