...
class WalletManager {
  ...
  /**
   * Get wallet address for a specific network (safe version that doesn't throw)
   * @param {string} network - Network identifier
   * @returns {string|null} Wallet address or null if not connected
   */
  getAddressSafe(network) {
    if (!this.isWalletConnected(network)) {
      return null;
    }
    try {
      return this.wallets.get(network).address;
    } catch (error) {
      defaultLogger.error(`Error getting address for ${network}: ${error.message}`);
      return null;
    }
  }
